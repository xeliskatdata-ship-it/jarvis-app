...
Tu parles à ${userName}. ${partnerInfo}${pronunciationHint}${cleanMemoriesContext}${personaReminder}`
    ...
    const { content: rawReply, toolsCalled, usage, model } = await chatWithTools(
      messages, tools, toolExecutors, { temperature: PERSONA.temperature ?? 0.7 }
    )
